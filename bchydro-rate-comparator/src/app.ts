...
function bindEvents() {
  ...
  els.dropZone.addEventListener("drop", event => {
    const files = Array.from((event as DragEvent).dataTransfer?.files || []).filter(file => /\.csv$/i.test(file.name) || file.type === "text/csv");
    processFiles(files);
  });
}
...
function parseBCHydroCsv(text, fileName) {
  ...
  if (rows.length < 2) throw new Error(`${fileName}: the CSV appears to be empty.`);

  const headers = rows[0].map((cell, index) => {
    const text = String(cell || "").trim();
    return index === 0 ? text.replace(/^\uFEFF/, "") : text;
  });
  const indices: Map<string, number> = new Map(headers.map((header, index): [string, number] => [header, index]));
  ...
  for (let i = 1; i < rows.length; i += 1) {
    ...
    const get = (name: string) => String(row[indices.get(name) as number] ?? "").trim();
...
function parseIntervalStart(value) {
  ...
  if (month < 1 || month > 12 || day < 1 || day > 31 || hour < 0 || hour > 23 || minute < 0 || minute > 59 || second !== 0) return null;
...
function parseNumber(value) {
  ...
  if (!trimmed || /^N\/A$/i.test(trimmed)) return NaN;
...
function analyseGroup(key, records) {
  ...
  for (const record of records) {
    if (!byTimestamp.has(record.timestampKey)) byTimestamp.set(record.timestampKey, []);
  ...
  for (const [timestampKey, entries] of byTimestamp.entries()) {
    ...
    for (const entry of entries) {
      if (!bySignature.has(entry.signature)) bySignature.set(entry.signature, []);
...
function evaluateCoverage(records, overlapIssues) {
  if (!records.length) return emptyCoverage();
  ...
  for (const record of records) {
    ...
    if (record.minute !== 0) minuteIssues.push(record.timestampKey);
  }
  ...
  for (const dateKey of dates) {
    ...
    for (const [timestampKey, expectedCount] of expectedCounts.entries()) {
      ...
      if (actual !== expectedCount) {
        complete = false;
        if (actual < expectedCount) missing.push({ timestampKey, missingCount: expectedCount - actual });
        if (actual > expectedCount) unexpected.push({ timestampKey, extraCount: actual - expectedCount });
      }
    }
    ...
    if (complete) completeDates.add(dateKey);
    else incompleteDates.push(dateKey);
  }
...
function buildCompleteRuns(dates, completeDates) {
  ...
  if (start) runs.push({ start, end: previous, days: daysBetweenInclusive(start, previous) });
  return runs;
}
...
function expectedTimestampsForDate(dateKey) {
  ...
  for (let hour = 0; hour < 24; hour += 1) {
    ...
    if (fallBack && hour === 1) result.push(`${dateKey} ${pad2(hour)}:00`);
  }
  return result;
}
...
function expectedCountForTimestamp(dateKey, hour) {
  const [year, month, day] = dateKey.split("-").map(Number);
  if (isSecondSundayOfMarch(year, month, day) && hour === 2) return 0;
  if (isFirstSundayOfNovember(year, month, day) && hour === 1) return 2;
  return 1;
}
...
function configureWindowControlsForSelectedGroup() {
  ...
  if (mode === "latest365") {
    ...
    if (!els.startDate.value && coverage.latest365) els.startDate.value = coverage.latest365.start;
    if (!els.endDate.value && coverage.latest365) els.endDate.value = coverage.latest365.end;
...
function periodForHour(hour) {
  if (hour >= 23 || hour < 7) return "overnight";
...
function renderFileSummary(fileReports, totalRecords) {
  ...
  const rowErrorItems = fileReports.flatMap(report =>
    report.rowErrors.slice(0, 8).map(error => `<li>${escapeHtml(report.fileName)} ${escapeHtml(error)}</li>`)
  );
...
function renderGapReport() {
  ...
  if (c.missing.length) {
    ...
    if (c.missing.length > 80) items.push(`<div class="gap-item">${formatInteger(c.missing.length - 80)} additional missing interval item${c.missing.length - 80 === 1 ? "" : "s"} not shown.</div>`);
  }
  if (c.unexpected.length) {
    ...
    if (c.unexpected.length > 80) items.push(`<div class="gap-item">${formatInteger(c.unexpected.length - 80)} additional unexpected interval item${c.unexpected.length - 80 === 1 ? "" : "s"} not shown.</div>`);
  }
...
function formatSignedCurrency(value) {
  if (Math.abs(value || 0) <= 0.005) return "$0";
